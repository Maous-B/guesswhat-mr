...
Le livrable sera une version *PDF* de votre REAMDE.adoc : sous PPHPStorm, `View|Appearence|Enter Presentation Mode` (c'est un mode `WYSIWYG`), le menu du haut dispose d'une commande d'export PDF : pour quitter ce mode, activer le menu en haut en déplaçant le curseur de la souris vers le haut de l'écran et choisir `Exit Preentation Mode`.
